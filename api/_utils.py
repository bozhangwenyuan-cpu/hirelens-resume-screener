        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "评分规则：先判断 0/1 一票否决项，再计算综合分。0/1 项包括岗位明确设置为 must 的核心能力、学历/经验门槛、关键资质；非敏感 must 缺失任一关键项原则上 conclusion 为“不匹配”。"
                    "综合分建议权重：硬性要求 55%，JD 职责匹配 25%，加分项 10%，风险扣分 10%。85 分及以上为优秀，70-84 为合格，50-69 为边界，50 以下不合格。"
                ...
                "content": json.dumps(
                    {
                        ...
                        "output_schema": {
                            ...
                            "score": "0-100 integer",
                            "matched_points": ["逐条列出岗位要求检查结果，每条格式：要求：...；证据：...；判断：符合/部分符合"],
                            "missing_points": ["逐条列出缺失项，每条格式：要求：...；缺失原因：...；影响：0/1否决/严重扣分/轻微扣分"],
